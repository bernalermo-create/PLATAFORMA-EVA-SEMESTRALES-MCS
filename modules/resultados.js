...
export function renderResultados(root) {
  ...
  root.innerHTML = `
    ...
    ${evals.length ? `
    <div class="form-row">
      <select id="rs-filter"><option value="">${admin ? 'Todas las evaluaciones' : 'Todas mis evaluaciones'}</option>${evals.map(e => `<option value="${e.id}">${e.nombre}</option>`).join('')}</select>
    </div>` : ''}
    ...
    <div class="card no-print">
      <div class="form-row">
        <select id="bo-curso">${cursos.map(c => `<option value="${c.id}">Grado ${c.grado}° ${c.paralelo} — ${c.jornada}/${c.seccion}</option>`).join('')}</select>
...
function generarBoletin(root, cursos) {
  ...
  if (!hojas.length) {
    boBox.innerHTML = `<div class="empty">No hay hojas generadas para Grado ${curso.grado}° ${curso.paralelo} en la Sesión ${sesion}.</div>`;
  ...
  boBox.innerHTML = `
    <div class="boletin-print">
      <div class="boletin-hd">
        ...
        <div>Boletín de resultados — Grado ${curso.grado}° ${curso.paralelo} · Sesión ${sesion}</div>
      </div>
      <table>
        <thead><tr>
          <th>Estudiante</th>
