...
const AREA_SHORT = {
  ...
  'Competencias Ciudadanas': 'C.Ciu', 'Lectura Crítica': 'L.Crít',
};

let _grado = null, _year = null, _tab = 'resumen', _aniosSeleccionados = null;
...
export function renderAnalisis(root) {
  ...
  root.querySelector('#an-grado').onchange = (e) => { _grado = parseInt(e.target.value); _aniosSeleccionados = null; renderAnalisis(root); };
...
function _renderTab(root, agg) {
  ...
  else if (_tab === 'anios') _tabEntreAnios(body);
...
function _tabEntreAnios(body) {
  const anios = store.aniosDisponibles(_grado);
  if (anios.length < 2) {
    body.innerHTML = `<div class="an-empty">Para comparar entre años hace falta que este grado tenga datos en al menos 2 años distintos. Por ahora solo hay: ${anios.join(', ') || 'ninguno'}.</div>`;
    return;
  }
  if (!_aniosSeleccionados) _aniosSeleccionados = new Set(anios);

  let html = `
    <p style="font-size:.8rem;color:var(--txt2);margin-bottom:1rem">Compara Grado ${_grado}° entre distintos años — útil para ver si las mismas áreas siguen débiles año tras año, o si mejoraron.</p>
    <div class="an-panel" style="margin-bottom:1rem">
      <div style="font-weight:700;font-size:.85rem;margin-bottom:.5rem">Años a comparar</div>
      <div style="display:flex;gap:1rem;flex-wrap:wrap">
        ${anios.map(y => `<label style="display:flex;align-items:center;gap:.3rem;font-size:.85rem">
          <input type="checkbox" class="an-anio-check" value="${y}" ${_aniosSeleccionados.has(y) ? 'checked' : ''}> ${y}
        </label>`).join('')}
      </div>
    </div>
  `;

  const aniosOrdenados = anios.filter(y => _aniosSeleccionados.has(y)).sort((a, b) => a - b);
  if (aniosOrdenados.length < 2) {
    html += `<div class="an-empty">Marca al menos 2 años para comparar.</div>`;
    body.innerHTML = html;
    _wireAnioChecks(body);
    return;
  }

  const datosPorAnio = aniosOrdenados.map(y => ({ year: y, agg: store.analisisAgregado(_grado, y) }));
  const combinadoDe = (agg) => {
    const c = { BAJO: 0, 'BÁSICO': 0, ALTO: 0, SUPERIOR: 0 };
    AREAS.forEach(a => NIVELES.forEach(n => c[n] += (agg[a].S1[n] || 0) + (agg[a].S2[n] || 0)));
    return c;
  };

  html += `<div class="an-panel" style="margin-bottom:1rem">
    <div style="font-weight:700;font-size:.85rem;margin-bottom:.7rem">Aprobación general (Básico + Alto + Superior) por año</div>
    <div style="display:flex;gap:1.8rem;flex-wrap:wrap;align-items:flex-end">
      ${datosPorAnio.map(({ year, agg }, i) => {
        const c = combinadoDe(agg);
        const T = _sumNivel(c);
        const aprob = T ? ((c['BÁSICO'] + c.ALTO + c.SUPERIOR) / T * 100) : 0;
        let delta = '';
        if (i > 0) {
          const cAnt = combinadoDe(datosPorAnio[i - 1].agg);
          const Tant = _sumNivel(cAnt);
          const aprobAnt = Tant ? ((cAnt['BÁSICO'] + cAnt.ALTO + cAnt.SUPERIOR) / Tant * 100) : 0;
          const diff = aprob - aprobAnt;
          delta = Tant ? `<div style="font-size:.72rem;color:${diff >= 0 ? 'var(--ok)' : 'var(--bad)'}">${diff >= 0 ? '▲' : '▼'} ${Math.abs(diff).toFixed(1)} pts vs ${datosPorAnio[i - 1].year}</div>` : '';
        }
        return `<div style="text-align:center">
          <div style="font-size:.72rem;color:var(--txt2)">${year}</div>
          <div style="font-size:1.6rem;font-weight:800">${T ? aprob.toFixed(1) : '—'}%</div>
          <div style="font-size:.7rem;color:var(--txt2)">${T} registro(s)</div>
          ${delta}
        </div>`;
      }).join('')}
    </div>
  ...
  AREAS.forEach(area => {
    html += `<div class="an-panel" style="margin-bottom:1rem"><div class="an-panel-title">📚 ${area}</div>`;
    let algunDato = false;
    datosPorAnio.forEach(({ year, agg }) => {
      const b = _sumBuckets(agg[area].S1, agg[area].S2);
      const T = _sumNivel(b);
      if (!T) { html += `<div style="font-size:.76rem;color:var(--txt2);margin-bottom:.5rem">${year}: sin resultados todavía.</div>`; return; }
      algunDato = true;
      html += `
        <div style="font-size:.76rem;color:var(--txt2);margin-bottom:.2rem">${year} (${T} reg.)</div>
        <div class="an-sbt" style="height:20px;margin-bottom:.6rem">${NIVELES.map(n => { const p = (b[n] || 0) / T * 100; return `<div class="an-sbs" style="width:${p.toFixed(1)}%;background:${NIVEL_COLOR[n]}">${p > 10 ? p.toFixed(0) + '%' : ''}</div>`; }).join('')}</div>
      `;
    });
    if (!algunDato) html += `<div style="font-size:.78rem;color:var(--txt2)">Sin resultados en ningún año seleccionado.</div>`;
    html += `</div>`;
  });
...
function _wireAnioChecks(body) {
  body.querySelectorAll('.an-anio-check').forEach(cb => cb.onchange = () => {
    const y = parseInt(cb.value);
    if (cb.checked) _aniosSeleccionados.add(y); else _aniosSeleccionados.delete(y);
    _tabEntreAnios(body);
  });
}
