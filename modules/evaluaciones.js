...
export function renderEvaluaciones(root) {
    ...
    <div class="grid grid-2">
      ${evals.length ? evals.map(ev => `
        <div class="card">
          ...
          <div style="margin:.5rem 0;font-size:.85rem;color:var(--txt2);display:flex;align-items:center;gap:.4rem;flex-wrap:wrap">
            <span>${ev.num_preguntas} pregunta(s) · Docente: ${escHTML(ev.docente) || '— sin asignar'}</span>
            <span>· Sesión:</span>
            ${admin
              ? `<select class="sesion-mini" data-ev="${ev.id}" style="padding:.1rem .3rem;font-size:.8rem">
                  <option value="1" ${ev.sesion === 1 ? 'selected' : ''}>1</option>
                  <option value="2" ${ev.sesion === 2 ? 'selected' : ''}>2</option>
                  <option value="" ${!ev.sesion ? 'selected' : ''}>sin definir</option>
                </select>`
              : `<b>${ev.sesion || 'sin definir'}</b>`}
          </div>
  ...
  root.querySelectorAll('.sesion-mini').forEach(sel => sel.onchange = () => {
    store.setSesionManual(sel.dataset.ev, sel.value);
    toast(sel.value ? `Sesión cambiada manualmente a ${sel.value}.` : 'Sesión vuelta a calcular automáticamente.');
    renderEvaluaciones(root);
  });
...
function renderConstructor(root, evaluacionId) {
    ...
    <div class="card">
      ...
      <div class="form-row" style="flex-direction:column;align-items:flex-start">
        <label style="display:flex;align-items:center;gap:.4rem;font-size:.8rem;color:var(--txt2)">
          <input type="checkbox" id="pq-lectura-on">
          📖 Esta pregunta usa un texto de lectura compartido (ej. "Responde las preguntas 21 a 25 según el siguiente texto") — se imprime una sola vez antes de la primera pregunta que lo use
        </label>
        <button type="button" class="btn sm sec hidden" id="pq-lectura-copiar" style="margin-top:.4rem">📋 Usar el mismo texto de la pregunta anterior</button>
        <div id="pq-lectura-campos" class="hidden" style="width:100%;margin-top:.5rem">
          <div class="form-row">
            <input id="pq-lectura-titulo" placeholder="Título de la lectura (opcional)">
          </div>
          <div class="form-row">
            <textarea id="pq-lectura-texto" rows="5" placeholder="Pega aquí el texto completo de la lectura..."></textarea>
          </div>
        </div>
      </div>
      <div class="form-row">
  ...
  const loadForEdit = (p) => {
    ...
    const tieneLectura = !!(p.lectura_texto);
    box.querySelector('#pq-lectura-on').checked = tieneLectura;
    box.querySelector('#pq-lectura-campos').classList.toggle('hidden', !tieneLectura);
    box.querySelector('#pq-lectura-titulo').value = p.lectura_titulo || '';
    box.querySelector('#pq-lectura-texto').value = p.lectura_texto || '';
    ['A','B','C','D'].forEach((l, i) => {
  ...
  box.querySelector('#pq-lectura-on').onchange = (e) => {
    box.querySelector('#pq-lectura-campos').classList.toggle('hidden', !e.target.checked);
  };

  // Si la pregunta anterior de esta evaluación ya tiene una lectura,
  // ofrece copiarla en vez de tener que pegar el mismo texto otra vez
  // por cada pregunta del grupo (21 a 25, por ejemplo).
  const anteriores = store.listPreguntas(evaluacionId);
  const anteriorConLectura = anteriores.length ? anteriores[anteriores.length - 1] : null;
  if (anteriorConLectura && anteriorConLectura.lectura_texto && !editingId) {
    const btnCopiar = box.querySelector('#pq-lectura-copiar');
    btnCopiar.classList.remove('hidden');
    btnCopiar.onclick = () => {
      box.querySelector('#pq-lectura-on').checked = true;
      box.querySelector('#pq-lectura-campos').classList.remove('hidden');
      box.querySelector('#pq-lectura-titulo').value = anteriorConLectura.lectura_titulo || '';
      box.querySelector('#pq-lectura-texto').value = anteriorConLectura.lectura_texto || '';
    };
  }
  ...
  box.querySelector('#pq-add').onclick = () => {
    ...
    const usaLectura = box.querySelector('#pq-lectura-on').checked;
    const lecturaTitulo = usaLectura ? box.querySelector('#pq-lectura-titulo').value.trim() : '';
    const lecturaTexto = usaLectura ? box.querySelector('#pq-lectura-texto').value.trim() : '';
    if (usaLectura && !lecturaTexto) { toast('Escribe el texto de la lectura, o desmarca la casilla.', 'warn'); return; }
    if (editingId) {
      store.updatePregunta(editingId, { enunciado, opciones, imagenUrl, competencia, componente, lecturaTitulo, lecturaTexto });
      toast('Pregunta actualizada.');
    } else {
      const mezclar = box.querySelector('#pq-mezclar').checked;
      store.addPregunta(evaluacionId, { enunciado, opciones, imagenUrl, competencia, componente, mezclar, lecturaTitulo, lecturaTexto });
