import { store, AREAS } from '../services/store.js';
      ...
      prev.innerHTML = `
        <div style="margin-top:.6rem;font-size:.85rem">
          ...
          <div style="max-height:180px;overflow:auto;background:var(--surf2);border-radius:8px;padding:.6rem;font-size:.78rem">
            ${data.slice(0, 40).map(f => `<div>${f.nombre} — ${f.jornada || '—'} — ${f.grados.join(',') || '—'} — ${f.areas.join(', ') || '—'}</div>`).join('')}
  ...
  return `
    <div class="card" style="border:2px solid var(--acc);margin-bottom:1rem">
      ...
      <table style="font-size:.85rem">
        ...
        <tbody>${lista.map(d => `<tr><td>${d.nombre}</td><td><code style="font-size:1rem;font-weight:800">${d.password}</code></td></tr>`).join('')}</tbody>
      </table>
