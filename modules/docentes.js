...
export function renderDocentes(root, bannerHtml = '') {
  ...
  root.innerHTML = `
    ...
    <div class="card">
      ...
      <div class="form-row" style="flex-wrap:wrap;gap:.4rem 1.2rem;margin-top:.4rem">
        <div style="font-size:.78rem;color:var(--txt2);width:100%">Jornada (puede ser más de una — hay docentes que dictan en mañana y tarde)</div>
        <label style="display:flex;align-items:center;gap:.3rem;font-size:.85rem"><input type="checkbox" class="dq-jornada" value="MANANA"> Mañana</label>
        <label style="display:flex;align-items:center;gap:.3rem;font-size:.85rem"><input type="checkbox" class="dq-jornada" value="TARDE"> Tarde</label>
        <label style="display:flex;align-items:center;gap:.3rem;font-size:.85rem"><input type="checkbox" class="dq-jornada" value="UNICA"> Única</label>
      </div>
      <div class="form-row" style="flex-wrap:wrap;gap:.4rem 1.2rem;margin-top:.6rem;align-items:center">
        <div style="font-size:.78rem;color:var(--txt2);width:100%">Sección — marca rápido todos los grados de Primaria o Bachillerato (puedes ajustar abajo)</div>
        <button type="button" class="btn sm sec" id="dq-sec-primaria">Marcar Primaria (2°-5°)</button>
        <button type="button" class="btn sm sec" id="dq-sec-bachillerato">Marcar Bachillerato (6°-11°)</button>
      </div>
        ...
        <tbody>
          ${docentes.map(d => `
            <tr>
              ...
              <td>${_jornadaLabel(d.jornadas)}</td>
  ...
  root.querySelector('#dq-sec-primaria').onclick = () => {
    [2,3,4,5].forEach(g => { const cb = root.querySelector(`.dq-grado[value="${g}"]`); if (cb) cb.checked = true; });
  };
  root.querySelector('#dq-sec-bachillerato').onclick = () => {
    [6,7,8,9,10,11].forEach(g => { const cb = root.querySelector(`.dq-grado[value="${g}"]`); if (cb) cb.checked = true; });
  };
  ...
  root.querySelector('#dq-add').onclick = () => {
    ...
    const jornadas = Array.from(root.querySelectorAll('.dq-jornada:checked')).map(c => c.value);
    try {
      const d = store.addDocente({ nombre, jornadas, grados, areas });
  ...
  root.querySelector('#dq-xl-preview').onclick = () => {
    ...
    reader.onload = (e) => {
      ...
      let data = rows.map(r => ({
        ...
        jornadas: _parseJornadas((r[1] || '').toString()),
      ...
      prev.innerHTML = `
        <div style="margin-top:.6rem;font-size:.85rem">
          ...
          <div style="max-height:180px;overflow:auto;background:var(--surf2);border-radius:8px;padding:.6rem;font-size:.78rem">
            ${data.slice(0, 40).map(f => `<div>${escHTML(f.nombre)} — ${_jornadaLabel(f.jornadas) || '—'} — ${f.grados.join(',') || '—'} — ${f.areas.map(escHTML).join(', ') || '—'}</div>`).join('')}
...
function _jornadaLabel(jornadas) {
  const map = { MANANA: 'Mañana', TARDE: 'Tarde', UNICA: 'Única' };
  return (jornadas || []).map(j => map[j] || j).join(', ');
}

function _exportarCSV(docentes) {
  const rows = [['Nombre','Jornada','Grados','Asignaturas','Contraseña','Estado']];
  docentes.forEach(d => rows.push([
    d.nombre, _jornadaLabel(d.jornadas), d.grados.join('|'), d.areas.join('|'), d.password, d.activo ? 'activo' : 'inactivo'
  ]));
...
function _parseJornadas(s) {
  const t = s.trim().toUpperCase();
  const jornadas = [];
  if (/MA[ÑN]ANA|^AM$|^M$/.test(t) || t.includes('MANANA')) jornadas.push('MANANA');
  if (/TARDE|^PM$|^T$/.test(t)) jornadas.push('TARDE');
  if (/[UÚ]NICA/.test(t)) jornadas.push('UNICA');
  if (/AMBAS|MA[ÑN]ANA.*TARDE|TARDE.*MA[ÑN]ANA/.test(t) && !jornadas.includes('MANANA')) { jornadas.push('MANANA'); jornadas.push('TARDE'); }
  return [...new Set(jornadas)];
}
