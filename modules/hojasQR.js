...
function generarCuadernillo(root, evaluacionIds, admin) {
  ...
  // ── Páginas de preguntas — flujo continuo entre áreas ──
  // Antes cada área SIEMPRE arrancaba en página nueva, así que una
  // página con menos de "porPagina" preguntas (ej. un área de 15,
  // dejando 10+5) se quedaba a medio llenar y la siguiente área
  // arrancaba en una página aparte de todos modos — es decir, cada
  // área que no fuera múltiplo exacto de 10 dejaba una página casi en
  // blanco, que es justo lo que se reportó. Ahora las preguntas de
  // todas las áreas se aplanan en una sola secuencia y se paginan
  // continuas, insertando el título de cada área donde corresponda
  // (aunque caiga a la mitad de una página) — cada página se llena
  // por completo salvo, como mucho, la última del cuadernillo.
  const porPagina = 10;
  const secuencia = [];
  let lecturaAbierta = null; // texto de la lectura que ya se imprimió, para no repetirla en cada pregunta del grupo
  bloques.forEach(({ ev, preguntas }) => {
    preguntas.forEach((p, i) => {
      const esNuevaLectura = !!(p.lectura_texto && p.lectura_texto !== lecturaAbierta);
      lecturaAbierta = p.lectura_texto || null;
      secuencia.push({
        p, ev,
        esInicioArea: i === 0,
        rangoArea: i === 0 ? `${preguntas[0]._abs} a ${preguntas[preguntas.length - 1]._abs}` : null,
        esNuevaLectura,
      });
    });
  });

  for (let i = 0; i < secuencia.length; i += porPagina) {
    const chunk = secuencia.slice(i, i + porPagina);
    const areasEnPagina = [...new Set(chunk.map(c => c.ev.area))].join(', ');
    ...
    page.innerHTML = `
      ...
      <div class="cuad-qbody">
        ${chunk.map(({ p, ev, esInicioArea, rangoArea, esNuevaLectura }) => `
          ${esInicioArea ? `<div class="cuad-area-title">${escHTML(ev.area)} (preguntas ${rangoArea})</div>` : ''}
          ${esNuevaLectura ? `
            <div class="lectura-block">
              ${p.lectura_titulo ? `<div class="lectura-titulo">${escHTML(p.lectura_titulo)}</div>` : ''}
              <div class="lectura-texto">${escHTML(p.lectura_texto)}</div>
            </div>` : ''}
          <div class="cuad-q">
      ...
      <div class="cuad-pagenum">Grado ${primera.grado}° · Sesión ${sesion} · ${areasEnPagina}</div>
    `;
...
function generarHojas(root, evaluacionIds, cursos) {
  ...
  estudiantes.forEach(est => {
    ...
    sheet.innerHTML = `
      ...
      <div class="hoja-estudiante">
        <div class="hoja-estudiante-datos">
          <div class="campo"><b>Nombres y apellidos</b><div class="valor">${escHTML(est.nombre)}</div></div>
          <div class="campo" style="margin-bottom:0"><b>Curso</b><div class="valor" style="max-width:160px">${curso.grado}° ${escHTML(curso.paralelo)}</div></div>
        </div>
        <div class="hoja-estudiante-qr">
          <div class="qr-box" data-qr="${hoja.id}"></div>
          <div class="qr-codigo">Código: ${hoja.codigo}</div>
        </div>
      </div>
      <div class="hoja-areas">
        ${bloques.map(({ ev, preguntas }) => `
          <div class="hoja-area-block">
            <div class="hoja-area-titulo">${escHTML(ev.area)}</div>
            ${preguntas.map(p => `
              <div class="omr-q">
                <b>${p._abs}</b>
    ...
    renderQR(sheet.querySelector(`[data-qr="${hoja.id}"]`), hoja.qr_payload, 125);
  });
