...
## Rediseño de hoja de respuestas, corrección del cuadernillo, y lectura compartida (julio 2026)

### Hoja de respuestas — ya no se ve desordenada
Causa real: cada columna de área repetía el nombre completo del estudiante, y con 3 áreas por sesión (el caso normal de la Sesión 1, no la excepción) el diseño las apretaba en tercios muy angostos. Ahora nombre, curso, QR y código aparecen **una sola vez** en un encabezado arriba, y las áreas quedan en una grilla que se adapta a 2 o 3 columnas sin apretujar. De paso se encontró y corrigió otra página en blanco: `.hoja-print` no le quitaba el margen al imprimir (a diferencia de `.cuad-page`, que sí lo tenía corregido desde antes) — ese margen de sobra empujaba el contenido a una segunda página casi vacía.

### Cuadernillo — la causa real de los espacios en blanco
No era un caso raro: **cada área que no fuera múltiplo exacto de 10 preguntas dejaba una página a medio llenar**, porque cada área siempre arrancaba en página nueva en vez de aprovechar el espacio que sobraba de la anterior. Con áreas de 15 preguntas (el caso normal del colegio), esto pasaba sistemáticamente. Ahora las preguntas de todas las áreas de la sesión fluyen continuas entre páginas — el título de cada área aparece donde corresponda, aunque caiga a la mitad de una página — y cada página se llena por completo. Verificado con un caso real (3 áreas, 40 preguntas): antes generaba 6 páginas con 2 de ellas a la mitad; ahora genera 5 páginas, todas completas.

### Bloque de lectura compartido
Al crear una pregunta, una casilla "📖 Esta pregunta usa un texto de lectura compartido" revela un campo de título y el texto completo. Si la pregunta anterior de la misma evaluación ya tiene una lectura, aparece un botón para copiarla en vez de tener que pegar el mismo texto en cada una de las 5 preguntas del grupo. Al imprimir el cuadernillo, el bloque de lectura sale **una sola vez**, antes de la primera pregunta que lo usa — no se repite en cada pregunta del grupo. Confirmado: no existe ni existirá aleatoriedad en el orden de las preguntas — la única mezcla es la de las letras de las opciones dentro de cada pregunta.

## Cierre de año, comparativo entre años, plan de mejoramiento (julio 2026)

Se agregó una pestaña nueva **📁 Informes** (solo administrador) y una pestaña **📅 Entre Años** dentro de Análisis. Hallazgo clave que simplificó todo el diseño: los datos ya estaban organizados por año (cada curso y evaluación tiene su `year`), así que **no hace falta borrar nada para "reiniciar"** — los años conviven en la misma base de datos sin chocar.

- **📅 Entre Años** (en Análisis): compara un grado entre 2 o más años — aprobación general con flecha de mejora/retroceso, y la distribución de niveles por área, año contra año.
- **📊 Informes anuales**: congela una foto fija de los resultados de un año (por grado y por área) — se guarda tal cual, así los datos originales cambien después. Se puede generar, ver el detalle, o borrar.
- **🔄 Año nuevo**: crea los cursos del año siguiente a partir de los del año base (subiendo de grado o manteniéndolo). A propósito **no** copia estudiantes automáticamente — la plataforma no sabe quién fue promovido, quién repite o quién es nuevo, eso lo decide el colegio subiendo el Excel real sobre los cursos ya creados. Sí copia el banco de evaluaciones completo (preguntas y niveles) como borrador, para no volver a digitarlas.
- **🎯 Plan de mejoramiento**: a partir de un informe anual, arma automáticamente el diagnóstico (qué grado+área quedó en Crítico o Atención, con los mismos umbrales que las Alertas de Análisis, y cuáles son fortalezas) y deja un espacio de texto por cada hallazgo para que el equipo académico escriba las acciones, responsables y metas — la plataforma no inventa recomendaciones pedagógicas, solo organiza el diagnóstico real.

Probado de punta a punta con datos de 2 años y 2 grados: el comparativo mostró correctamente la mejora entre años, el informe capturó ambos grados, el año nuevo promovió los cursos correctamente (Grado 6°→7°, Grado 9°→10°) y copió las evaluaciones, y el plan de mejoramiento marcó como Crítico solo el grado que de verdad tenía 80% en Nivel Bajo, sin falsos positivos en el que estaba bien.
