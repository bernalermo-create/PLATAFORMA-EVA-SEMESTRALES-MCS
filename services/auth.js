...
export function login(role, user, pwd, docentes) {
  ...
  // Compatibilidad hacia atrás: si el nombre no está en el padrón
  // (aún no migrado), sigue funcionando con la contraseña compartida,
  // sin restricción de alcance — igual que antes de este cambio.
  if (pwd !== getTeacherPwd()) return { ok: false, error: 'Contraseña de docente incorrecta. Si ya te registraron con usuario individual, usa esa contraseña.' };
  sessionStorage.setItem(SESSION_KEY, JSON.stringify({ role: 'docente', user: nombreN, grados: [], areas: [] }));
  return { ok: true };
}
