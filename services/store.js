...
// subidos a Sheets. Se guarda en localStorage (no solo en memoria)
// porque debe sobrevivir a un recargo/cierre de la pestaña justo
// después de escanear — ver comentario en initRemote().
const PENDING_KEY = 'pev_pending_push_v1';
...
function load() {
  ...
  return { cursos: [], estudiantes: [], evaluaciones: [], preguntas: [], hojas: [], resultados: [], docentes: [], analisisTexto: [], informesAnuales: [] };
}
...
function _ensureShape(db) {
  ...
  if (!db.informesAnuales) db.informesAnuales = [];
...
export const store = {
  ...
  // nombre).
  async initRemote() {
    // Si la pestaña se recargó (o la app se cerró) justo después de
    // guardar algo localmente, sin que el envío a Sheets alcanzara a
    // confirmarse (~700ms + red), este pull normal traería la versión
    // vieja de la nube y borraría en silencio ese cambio recién hecho
    // — justo el caso real de un docente escaneando hojas seguidas en
    // el celular con wifi débil. Si hay un envío pendiente reciente
    // (menos de 5 min — más viejo que eso, se asume una pestaña
    // abandonada y se prioriza no pisar cambios de otros con datos
  ...
  addPregunta(evaluacionId, { enunciado, competencia, componente, opciones, imagenUrl, mezclar, lecturaTitulo, lecturaTexto }) {
    ...
    const p = {
      ...
      lectura_titulo: lecturaTitulo || '', lectura_texto: lecturaTexto || '',
  ...
  updatePregunta(id, { enunciado, competencia, componente, opciones, imagenUrl, lecturaTitulo, lecturaTexto }) {
    ...
    p.lectura_titulo = lecturaTitulo || '';
    p.lectura_texto = lecturaTexto || '';
  ...
  // Años con datos reales para un grado — para el comparativo entre
  // años (no tiene sentido ofrecer comparar un año que no tiene ni
  // una evaluación cargada para ese grado).
  aniosDisponibles(grado) {
    const g = parseInt(grado);
    return [...new Set(DB.evaluaciones.filter(e => e.grado === g).map(e => e.year))].sort();
  },
  ...
  // Congela una "foto" de los resultados de un año — a diferencia de
  // analisisAgregado() (que siempre recalcula en vivo), esto queda
  // guardado tal cual, para poder compararlo con años futuros aunque
  // los datos originales cambien o se limpien más adelante.
  generarInformeAnual(year) {
    const y = parseInt(year);
    const grados = [...new Set(DB.evaluaciones.filter(e => e.year === y).map(e => e.grado))].sort((a, b) => a - b);
    const vacio = () => ({ BAJO: 0, 'BÁSICO': 0, ALTO: 0, SUPERIOR: 0 });
    const sumar = (b) => NIVELES.reduce((s, n) => s + (b[n] || 0), 0);
    const aprob = (b, t) => t ? ((b['BÁSICO'] + b.ALTO + b.SUPERIOR) / t * 100) : 0;

    const porGrado = grados.map(grado => {
      const agg = this.analisisAgregado(grado, y);
      const combinado = vacio();
      const porArea = AREAS.map(area => {
        const b = vacio();
        NIVELES.forEach(n => { b[n] = (agg[area].S1[n] || 0) + (agg[area].S2[n] || 0); combinado[n] += b[n]; });
        const t = sumar(b);
        return { area, total: t, niveles: b, aprobacion: aprob(b, t) };
      }).filter(a => a.total > 0);
      const t = sumar(combinado);
      return { grado, total: t, niveles: combinado, aprobacion: aprob(combinado, t), porArea };
    });

    const totalEscuela = vacio();
    porGrado.forEach(g => NIVELES.forEach(n => totalEscuela[n] += g.niveles[n]));
    const tEscuela = sumar(totalEscuela);

    const informe = {
      id: uid('informe'), year: y, generado_en: new Date().toISOString(),
      totalRegistros: tEscuela,
      niveles: totalEscuela,
      aprobacion: aprob(totalEscuela, tEscuela),
      porGrado,
    };
    // Si ya existía un informe para ese año, se reemplaza (regenerar =
    // actualizar con los datos más recientes, no acumular duplicados).
    DB.informesAnuales = (DB.informesAnuales || []).filter(i => i.year !== y);
    DB.informesAnuales.push(informe);
    scheduleSync();
    return informe;
  },
  listInformesAnuales() {
    return (DB.informesAnuales || []).slice().sort((a, b) => b.year - a.year);
  },
  getInformeAnual(year) {
    return (DB.informesAnuales || []).find(i => i.year === parseInt(year)) || null;
  },
  deleteInformeAnual(id) {
    DB.informesAnuales = (DB.informesAnuales || []).filter(i => i.id !== id);
    scheduleSync();
  },
  ...
  _debugReset() { DB = { cursos: [], estudiantes: [], evaluaciones: [], preguntas: [], hojas: [], resultados: [], docentes: [], analisisTexto: [], informesAnuales: [] }; scheduleSync(); }
};
