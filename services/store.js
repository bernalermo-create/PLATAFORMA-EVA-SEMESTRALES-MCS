...
export const AREAS = ['Matemáticas', 'Ciencias Naturales', 'Inglés', 'Competencias Ciudadanas', 'Lectura Crítica', 'Español'];
...
export const store = {
  ...
  addDocente({ nombre, jornada, grados, areas }) {
    if (!DB.docentes) DB.docentes = [];
    ...
    const d = {
      id: uid('doc'), nombre: nombreN, jornada: jornada || '',
  ...
  // filas: [{nombre, jornada, grados:[...], areas:[...]}] — nombres
  ...
  importDocentesMasivo(filas) {
    ...
    (filas || []).forEach(f => {
      ...
      const d = {
        id: uid('doc'), nombre: nombreN, jornada: f.jornada || '',
  ...
  updateDocente(id, { jornada, grados, areas, activo }) {
    const d = this.getDocente(id);
    if (!d) return null;
    if (jornada !== undefined) d.jornada = jornada;
  ...
  analisisAgregado(grado, year) {
    ...
    const porArea = {};
    AREAS.forEach(area => { porArea[area] = { S1: vacio(), S2: vacio(), competencias: {} }; });
      ...
      DB.resultados.filter(r => r.evaluacion_id === ev.id).forEach(r => {
        ...
        if (r.por_competencia) {
          Object.entries(r.por_competencia).forEach(([comp, counts]) => {
            if (!porArea[ev.area].competencias[comp]) porArea[ev.area].competencias[comp] = vacio();
            NIVELES.forEach(n => { porArea[ev.area].competencias[comp][n] += (counts[n] || 0); });
          });
        }
      });
    });
